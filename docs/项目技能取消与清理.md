...
> 历史设计记录。当前 `remove` 命令位于 `internal/clis/skill-hub/remove.go`，项目状态修改经 ProjectState port 完成。
...
*   **边界清晰性**：`remove` 只清理当前项目中的技能使用记录；若整个项目路径已失效，应使用 `skill-hub prune` 清理残留项目状态。
