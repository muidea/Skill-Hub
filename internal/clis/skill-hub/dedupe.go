...
该命令默认只报告，不修改文件。`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return runDedupe(args[0], projectport.DedupeOptions{
			Canonical: canonical,
			Strategy:  strategy,
			Report:    true,
		}, jsonOutput)
...
func runDedupe(scope string, opts projectport.DedupeOptions, jsonOutput bool) error {
	absScope, err := filepath.Abs(scope)
	if err != nil {
		return errors.Wrap(err, "解析scope失败")
	}

	var report *projectport.DuplicateReport
	if client, ok := hubClientIfAvailable(); ok {
		...
		report, err = localRuntime.Project().Dedupe(absScope, opts)
...
func renderDedupeReport(report *projectport.DuplicateReport) {
	if report == nil {
