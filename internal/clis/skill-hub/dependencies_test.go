...
func TestRequireInitOnly(t *testing.T) {
	t.Run("已初始化时返回带Cwd的上下文", func(t *testing.T) {
		...
		if ctx.ProjectState != nil {
			t.Error("RequireInitOnly() expected nil ProjectState")
		}
	})
}
...
func TestRequireInitAndWorkspace(t *testing.T) {
  ...
  "` + projectDir + `": {
    ...
    "skills": {}
		...
		if ctx == nil || ctx.Cwd == "" || ctx.ProjectState == nil {
			t.Errorf("RequireInitAndWorkspace() expected full context, got Cwd=%q ProjectState=%v",
				ctx.Cwd, ctx.ProjectState != nil)
		}
	})
}
