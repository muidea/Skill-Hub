...
func init() {
	listCmd.Flags().Bool("verbose", false, "显示详细信息，包括技能描述、版本、标签和作者等")
	...
	if client, ok := hubClientIfAvailable(); ok {
		...
		if len(matchers) == 0 {
			if len(repoFilters) == 0 {
				skillsMetadata, err = localRuntime.Repository().ListSkills(nil)
				if err != nil {
					return errors.Wrap(err, "获取技能列表失败")
				}
			} else {
				availableRepos, listErr := localRuntime.Repository().List(false)
				if listErr != nil {
					return errors.Wrap(listErr, "获取仓库列表失败")
				}
				if err := validateRepoFilters(availableRepos, repoFilters); err != nil {
					return err
				}
				skillsMetadata, err = localRuntime.Repository().ListSkills(repoFilters)
				if err != nil {
					return errors.Wrap(err, "获取技能列表失败")
				}
			}
		} else {
			skillsMetadata, err = localRuntime.Repository().FindSkills(patterns, repoFilters)
			if err != nil {
...
func validateRepoFilters(availableRepos []repositoryport.RepositoryConfig, repoFilters []string) error {
...
func renderSkillList(skillsMetadata []spec.SkillMetadata, repoFilters []string, patterns []string, verbose bool) {
	...
	if verbose {
		...
		totalWidth := widths.idMin + widths.nameMin + widths.versionMin + widths.repoMin + 3
