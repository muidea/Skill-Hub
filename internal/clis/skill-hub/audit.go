...
import (
	...
	projectport "github.com/muidea/skill-hub/internal/pkg/projectlifecycleport"
...
默认 scope 为 .agents/skills，默认输出 Markdown 到标准输出。`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return runAudit(projectport.AuditOptions{
			Scope:       scope,
			Canonical:   canonical,
...
func runAudit(opts projectport.AuditOptions, format, output string) error {
	cwd, err := os.Getwd()
	if err != nil {
	...
	var report *projectport.AuditReport
	if client, ok := hubClientIfAvailable(); ok {
		...
		report, err = localRuntime.Project().Audit(opts)
...
func renderAuditPayload(report *projectport.AuditReport, format string) ([]byte, error) {
...
func renderAuditMarkdown(report *projectport.AuditReport) string {
	if report == nil {
