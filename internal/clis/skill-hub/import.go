...
使用 --dry-run 可预览将要执行的登记、修复和归档动作。`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		opts := projectport.ImportOptions{
...
func runImport(skillsDir string, opts projectport.ImportOptions) error {
	cwd, err := os.Getwd()
	if err != nil {
	...
	var summary *projectport.ImportSummary
	if client, ok := hubClientIfAvailable(); ok {
		...
		summary, err = localRuntime.Project().Import(ctx.Cwd, skillsDir, opts)
...
func renderImportSummary(summary *projectport.ImportSummary) {
	if summary == nil {
		fmt.Println("\n=== import summary ===")
		fmt.Println("未返回导入摘要")
		return
