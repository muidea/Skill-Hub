...
func (s *Service) Upgrade(ctx context.Context, opts Options) (*Result, error) {
	...
	daemonName := "skill-hubd"
	if opts.GOOS == "windows" {
		binaryName = "skill-hub.exe"
		daemonName = "skill-hubd.exe"
	}
	newBinary, err := findExtractedBinary(extractDir, binaryName)
	if err != nil {
		return nil, err
	}
	newDaemon, err := findExtractedBinary(extractDir, daemonName)
	if err != nil {
		return nil, err
	}
	...
	if err := validateBinaryVersion(ctx, newDaemon, result.TargetVersion); err != nil {
		return nil, err
	}
	if err := replaceBinaries([]binaryReplacement{
		{source: newBinary, target: opts.InstallPath},
		{source: newDaemon, target: siblingDaemonPath(opts.InstallPath, daemonName)},
	}); err != nil {
		return nil, err
	}
...
func parseVersionOutput(output string) string {
	for _, line := range strings.Split(output, "\n") {
		...
		if strings.HasPrefix(line, "skill-hub version ") || strings.HasPrefix(line, "skill-hubd version ") {
			value := strings.TrimPrefix(line, "skill-hub version ")
			value = strings.TrimPrefix(value, "skill-hubd version ")
...
func siblingDaemonPath(installPath, daemonName string) string {
	return filepath.Join(filepath.Dir(installPath), daemonName)
}

// replaceBinaries installs the CLI and daemon as one recoverable transaction.
// A failure restoring either target rolls back every already-replaced binary.
func replaceBinaries(replacements []binaryReplacement) error {
	if len(replacements) == 0 {
		return errors.NewWithCode("upgrade", errors.ErrFileNotFound, "没有可替换的二进制")
	}
	...
	for index := range replacements {
		...
		item.tmp = filepath.Join(filepath.Dir(item.target), "."+filepath.Base(item.target)+".new."+pid)
		item.backup = filepath.Join(filepath.Dir(item.target), "."+filepath.Base(item.target)+".old."+pid)
		_ = os.Remove(item.tmp)
		_ = os.Remove(item.backup)
		if err := copyFile(item.source, item.tmp, 0755); err != nil {
			for cleanup := range replacements[:index+1] {
				_ = os.Remove(replacements[cleanup].tmp)
			}
			return err
		}
	}
	...
	for index := range replacements {
		...
		if _, err := os.Stat(item.target); err == nil {
			if err := os.Rename(item.target, item.backup); err != nil {
				rollbackBinaries(replacements[:index])
				return errors.WrapWithCode(err, "upgrade", errors.ErrFileOperation, "备份当前二进制失败")
			}
		}
		if err := os.Rename(item.tmp, item.target); err != nil {
			if _, backupErr := os.Stat(item.backup); backupErr == nil {
...
func rollbackBinaries(replacements []binaryReplacement) {
