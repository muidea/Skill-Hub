...
func TestUpgradeDownloadsVerifiesAndReplacesBinary(t *testing.T) {
	...
	daemonContent, err := os.ReadFile(filepath.Join(filepath.Dir(installPath), "skill-hubd"))
	if err != nil {
		t.Fatalf("read installed daemon: %v", err)
	}
	if !strings.Contains(string(daemonContent), "skill-hubd version 0.9.0") {
		t.Fatalf("installed daemon was not replaced: %q", string(daemonContent))
	}
}
...
func writeReleaseArchive(t *testing.T, archivePath string, version string) {
	...
	for _, name := range []string{"skill-hub", "skill-hubd"} {
		binary := fmt.Sprintf("#!/bin/sh\nif [ \"$1\" = \"--version\" ]; then echo \"%s version %s (commit: test, built: now)\"; exit 0; fi\n", name, version)
