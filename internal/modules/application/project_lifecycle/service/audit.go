...
func (p *ProjectLifecycle) Audit(opts AuditOptions) (*AuditReport, error) {
	...
	if err != nil {
		...
		report.DefaultRepository = defaultRepo
	}
	...
	statusSummary, statusErr := projectstatusservice.New(p.projectState, p.repository).Inspect(absProjectPath, "")
	if statusErr != nil {
...
func (p *ProjectLifecycle) inspectRemotePush(defaultRepoName string) RemotePushReport {
	...
	repoPath, err := p.repository.Path(defaultRepoName)
	if err != nil {
