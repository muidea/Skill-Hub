...
func newProjectApply(t *testing.T) *ProjectApply {
	t.Helper()
	hub := event.NewHub(8)
	...
	return New(
		projectstateport.New(hub, "project_apply_test"),
		repositoryport.NewProjectSource(hub, "project_apply_test"),
	)
}

func newProjectStatusForTest(t *testing.T) *projectstatusservice.ProjectStatus {
	t.Helper()
	hub := event.NewHub(8)
	background := task.NewBackgroundRoutine(2)
	projectState := projectstatebiz.New(hub, background, projectstateservice.New())
	...
	return projectstatusservice.New(
		projectstateport.New(hub, "project_apply_test"),
		repositoryport.NewProjectSource(hub, "project_apply_test"),
	)
}
...
func TestProjectApply_ApplyOpenCodeProject(t *testing.T) {
	...
	result, err := newProjectApply(t).Apply(projectDir, "", false, false)
	if err != nil {
...
func TestProjectApply_ApplySpecificOutdatedSkillRefreshesFromRepository(t *testing.T) {
	...
	result, err := newProjectApply(t).Apply(projectDir, "demo-skill", false, false)
	if err != nil {
		t.Fatalf("Apply returned error: %v", err)
	}
	...
	status, err := newProjectStatusForTest(t).Inspect(projectDir, "demo-skill")
	if err != nil {
