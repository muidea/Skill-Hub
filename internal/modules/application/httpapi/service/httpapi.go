...
type HTTPAPI struct{ runtimeSvc *localruntime.Runtime }

func New() *HTTPAPI {
	return &HTTPAPI{runtimeSvc: localruntime.New()}
}

// NewWithRuntime reuses a process-level capability assembly. It is used by
// skill-hubd so HTTP handlers share the framework EventHub and Blocks.
func NewWithRuntime(runtime *localruntime.Runtime) *HTTPAPI {
	return &HTTPAPI{runtimeSvc: runtime}
}

func (h *HTTPAPI) Close() {
	if h != nil && h.runtimeSvc != nil {
		h.runtimeSvc.Close()
		h.runtimeSvc = nil
	}
}
...
func (h *HTTPAPI) handleRepos(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		repos, err := h.runtimeSvc.Repository().List(true)
		if err != nil {
			writeWrappedError(w, err)
			return
		}
		defaultRepo, _ := h.runtimeSvc.Repository().Default()
		defaultRepoName := ""
		if defaultRepo != nil {
	...
	case http.MethodPost:
		...
		if err := h.runtimeSvc.Repository().Add(repoCfg); err != nil {
			writeWrappedError(w, err)
			return
		}
...
func (h *HTTPAPI) handleRepoActions(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 1 && r.Method == http.MethodDelete {
		if err := h.runtimeSvc.Repository().Remove(name); err != nil {
			writeWrappedError(w, err)
			return
		}
	...
	switch parts[1] {
	case "sync":
		result, err := h.runtimeSvc.Repository().Sync(name)
		if err != nil {
			writeWrappedError(w, err)
			return
		}
		...
		return
	case "enable":
		if err := h.runtimeSvc.Repository().Enable(name); err != nil {
			writeWrappedError(w, err)
			return
		}
	case "disable":
		if err := h.runtimeSvc.Repository().Disable(name); err != nil {
			writeWrappedError(w, err)
			return
		}
	case "set-default":
		if err := h.runtimeSvc.Repository().SetDefault(name); err != nil {
			writeWrappedError(w, err)
			return
		}
	default:
...
func (h *HTTPAPI) handleSkillRepositoryStatus(w http.ResponseWriter, r *http.Request) {
	...
	status, err := h.runtimeSvc.Git().SkillRepositoryStatus()
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleCheckSkillRepository(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Git().CheckSkillRepositoryUpdates()
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleSyncSkillRepository(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.runtimeSvc.Git().SyncSkillRepositoryAndRefresh(); err != nil {
		writeWrappedError(w, err)
		return
	}
	skillCount := 0
	if defaultRepo, err := h.runtimeSvc.Repository().Default(); err == nil && defaultRepo != nil {
		if skills, err := h.runtimeSvc.Repository().ListSkills([]string{defaultRepo.Name}); err == nil {
...
func (h *HTTPAPI) handlePushSkillRepository(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.runtimeSvc.Git().PushSkillRepositoryChanges(message); err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) pushSkillRepositoryPreview() (httpapibiz.PushSkillRepositoryPreviewData, error) {
	...
	updates, err := h.runtimeSvc.Git().CheckSkillRepositoryUpdates()
	if err != nil {
		return httpapibiz.PushSkillRepositoryPreviewData{}, err
	}
...
func (h *HTTPAPI) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	items, err := h.runtimeSvc.Skill().SearchRemote(keyword, limit)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleSkills(w http.ResponseWriter, r *http.Request) {
	...
	skills, err := h.runtimeSvc.Repository().ListSkills(repoNames)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleFindSkills(w http.ResponseWriter, r *http.Request) {
	...
	items, err := h.runtimeSvc.Repository().FindSkills(req.Patterns, req.RepoNames)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleSkillActions(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasSuffix(path, "/candidates") {
		...
		items, err := h.runtimeSvc.Repository().FindSkill(skillID)
		if err != nil {
			writeWrappedError(w, err)
			return
		}
	...
	item, err := h.runtimeSvc.Repository().LoadSkill(skillID, repoName)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleProjects(w http.ResponseWriter, r *http.Request) {
	...
	projects, err := h.runtimeSvc.Project().ListProjects()
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleProjectActions(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 1 {
		project, err := h.runtimeSvc.Project().GetProject(id)
		if err != nil {
			writeWrappedError(w, err)
			return
		}
	...
	if len(parts) == 2 && parts[1] == "skills" {
		skills, err := h.runtimeSvc.Project().ListProjectSkills(id)
		if err != nil {
			writeWrappedError(w, err)
			return
		}
...
func (h *HTTPAPI) handleProjectStatus(w http.ResponseWriter, r *http.Request) {
	...
	status, err := h.runtimeSvc.Project().Inspect(projectPath, skillID)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleUseSkill(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Enable(req.ProjectPath, req.SkillID, req.Repository, req.Variables)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleRegisterSkill(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Register(req.ProjectPath, req.SkillID, req.SkipValidate)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleImportSkills(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Import(req.ProjectPath, req.SkillsDir, req.Options)
...
func (h *HTTPAPI) handleDedupeSkills(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Dedupe(req.Scope, req.Options)
	if err != nil && result == nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleSyncCopies(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().SyncCopies(req.Options)
	if err != nil && result == nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleLintPaths(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().LintPaths(req.Options)
	if err != nil && result == nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleValidateProjectSkills(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Validate(req.Options)
	if err != nil && result == nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleAuditProjectSkills(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Audit(req.Options)
	if err != nil && result == nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleApplyProject(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().Apply(req.ProjectPath, req.SkillID, req.DryRun, req.Force)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleGlobalStatus(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Global().Inspect(skillID, agents)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleUseGlobalSkill(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Global().Enable(req.SkillID, req.Repository, req.Agents, req.Variables)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleApplyGlobal(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Global().Apply(req.SkillID, req.Agents, req.DryRun, req.Force)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleGlobalSkillActions(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Global().Remove(skillID, parseAgentQuery(r.URL.Query()), force)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handlePreviewFeedback(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().PreviewFeedback(req.ProjectPath, req.SkillID)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
...
func (h *HTTPAPI) handleApplyFeedback(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.runtimeSvc.Project().ApplyFeedback(req.ProjectPath, req.SkillID)
	if err != nil {
		writeWrappedError(w, err)
		return
	}
